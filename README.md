---------------------------------------------------------------------------------------
|本项目是fork的甬哥的项目，只保留了H2节点，有证书。                                  |
|纯属娱乐，请支持原创！                                                               |
---------------------------------------------------------------------------------------
### VPS专用一键脚本如下(注意：这里是修改了地址为本项目的地址而不是甬哥的地址，甬哥的请自行点击链接去看甬哥的)：快捷方式：```sb```
```
bash <(wget -qO- https://raw.githubusercontent.com/jawaoo/singboxm/main/sb.sh)
```
或者
```
bash <(curl -Ls https://raw.githubusercontent.com/jawaoo/singboxm/main/sb.sh)
```
